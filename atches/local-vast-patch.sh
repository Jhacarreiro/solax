...
**Título do commit:**
```
Add local patch script for X1-VAST behind HTTPS proxy
```

**Descrição:**
```
Shell script that patches the solax library inside the HA container
to skip full discovery and target X1HybridGen4 directly.

Needed when the WiFi+LAN 2.0 dongle is behind an HTTPS reverse proxy
and cannot handle 22+ parallel discovery requests.
